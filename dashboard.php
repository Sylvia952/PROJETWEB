...
// ==== ALERTES AUTOMATIQUES (faible stock) ====
$seuil = 5; // seuil de stock faible
...
foreach ($produitsFaibleStock->fetchAll(PDO::FETCH_ASSOC) as $prod) {
    $msg = "Low stock: '{$prod['nom']}' ({$prod['quantite']} units remaining)";
    $stmt = $pdo->prepare("SELECT id FROM alertes WHERE message = ? AND lue = 0");
    $stmt->execute([$msg]);
    if (!$stmt->fetch()) {
...
$produitsFaibleStockCount = $pdo->query("SELECT COUNT(*) FROM produits WHERE quantite <= $seuil")->fetchColumn();

$totalVentes = $pdo->query("SELECT SUM(prix_total) FROM ventes")->fetchColumn();
$totalVentes = $totalVentes ?: 0; // éviter null
$totalClients = $pdo->query("SELECT COUNT(*) FROM clients")->fetchColumn();

// ==== ALERTES RÉCENTES ====
$recentAlerts = $pdo->query("SELECT * FROM alertes ORDER BY date_alert DESC LIMIT 5")->fetchAll(PDO::FETCH_ASSOC);

// ==== DONNÉES POUR LES GRAPHIQUES (exemple) ====
// Total ventes par mois (pour chart.js)
...
// Catégories de produits (pour diagramme en camembert)
...
// Transformer les données en JSON pour JS
$ventesParMoisJSON = json_encode($ventesParMois);
...
<title>Cold Management - Dashboard</title>
        ...
        <main class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div class="bg-white rounded-lg shadow p-6 border-l-4 border-red-500">
                    <div class="flex justify-between items-start">
                        <div>
                            <p class="text-sm text-gray-500">Produits Faible Stock</p>
            ...
            <div class="bg-white rounded-lg shadow overflow-hidden">
                <div class="p-4 border-b border-gray-200">
                    <h3 class="font-semibold text-blue-800 flex items-center"><i data-feather="bell" class="mr-2"></i> Recent Alerts</h3>
                </div>
                    ...
                    <?php foreach ($recentAlerts as $alert): ?>
                        <div class="p-4 hover:bg-blue-50 flex items-start">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500"><?= isset($alert['date_alert']) ? date('d M Y H:i', strtotime($alert['date_alert'])) : '' ?></p>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
